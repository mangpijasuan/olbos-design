...
**Status:** Vision captured. Phase 0 audit complete. **Phase 1a (rebrand & cleanup) complete. Phase 1b (event-type registry) complete.** Phase 1c–1e (theme/fabric split, generic renderer, organizations) not yet started — awaiting a scoping decision (see [Recommended Path Forward](#recommended-path-forward)).
...
Given the master prompt's own instruction ("do NOT immediately rebuild... first inspect... produce an audit... then begin implementation incrementally," §2/§76), I've stopped here rather than starting a multi-week rewrite unprompted. Before writing code, it's worth deciding:

1. **Where to start** — the plan above (1a → 1e) is one reasonable slicing, but 1a (rebrand) is nearly free while 1b–1e (event-type registry, theme/fabric split, generic renderer, organizations) is genuinely substantial engineering. Do you want to start with 1a now, or jump straight to the schema work?
2. **How far to go before the next check-in** — given §73's quality-gate discipline, I'd plan to stop and show you working results after each lettered slice (1a, then 1b, etc.) rather than batching all of Phase 1 into one delivery.
3. **What "OLBOS DESIGN" as a name means for the shipped demo data** — the 5 live demo invitations, README, and marketing copy all currently say "Olbos Event"; renaming touches a lot of already-verified surface area.
